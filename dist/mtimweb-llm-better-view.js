...
\f\r"'\`<>=]|("|')|))|$)`, "g"), g = /'/g, $ = /"/g, y = /^(?:script|style|textarea|title)$/i, x = (t2) => (i2, ...s2) => ({ _$litType$: t2, strings: i2, values: s2 }), b = x(1), E = Symbol.for("lit-noChange"), A = Symbol.for("lit-nothing"), C = new WeakMap(), P = l.createTreeWalker(l, 129);
  function V(t2, i2) {
  ...
  function aggregateChoices(events) {
    ...
    return Array.from(choiceMap.values()).map((agg) => ({
      ...
      role: agg.role,
      content: agg.content,
      tool_calls: Array.from(agg.tool_calls.values()).sort((a2, b2) => a2.index - b2.index),
...
walkTokens(tokens, callback) {
  ...
  function renderChoiceTextContent(content) {
    ...
    if (_isJSON) {
      return b`<div data-format='text-as-json' style="white-space: pre; font-family: monospace; overflow-x: auto;"><pre>${JSON.stringify(JSON.parse(content), null, 2)}</pre></div>`;
    }
    let isXml = isXmlFragment(content);
    let isMarkdown = content.startsWith("#") || content.includes("\n```") || content.includes("\n# ") || content.includes("\n## ") || content.includes("\n# ") || content.includes("\n### ") || content.includes("\n1. ") || content.includes("\n- ");
    if (isXml && !isMarkdown) {
      return b`<div data-format='xml' style="white-space: pre; font-family: monospace; overflow-x: auto;">${content}</div>`;
    }
  ...
  function renderToolMessage(content) {
    try {
      ...
      const htmls = toolArr.map((toolObj) => {
        ...
        return b`<div  style="white-space: pre; font-family: monospace;">${content}</div>`;
      });
  ...
  function renderToolChoiceArgument(_arguments) {
    if (typeof _arguments === "object") {
      return b`<div class="json-content"><pre>${JSON.stringify(_arguments, null, 2)}</pre></div>`;
    }
    try {
      const toolObj = JSON.parse(_arguments);
      return b`<div class="json-content"><pre>${JSON.stringify(toolObj, null, 2)}</pre></div>`;
    } catch (e2) {
    }
    return b`<div  style="white-space: pre; font-family: monospace;">${_arguments}</div>`;
  }
  const joinedWithHr = (sections) => sections.flatMap(
    (section, index, arr) => index < arr.length - 1 ? [section, b`<hr />`] : [section]
);
  ...
  const renderMessageContent = (message) => {
    if (message.role === "tool") {
      return b`<div class="prose" data-type="tool">${renderToolMessage(message.content)}</div>`;
    } else if (typeof message.content === "string") {
      return b`<div class="prose" data-format="string">${renderChoiceTextContent(message.content)}</div>`;
    } else if (Array.isArray(message.content)) {
      return b`<div data-format="array">${message.content.map((item) => renderMessageContent(item))}</div> `;
    } else if (isAnthropicContent(message)) {
      return renderAnthropicContent(message);
    } else {
      return b`<div class="json-content" data-format="object">${JSON.stringify(message.content, null, 2)}</div>`;
    }
  };
  ...
  function renderAnthropicContent(content) {
    if (content.type === "text") {
      return b`<div class="prose" data-format="string" data-content-type="anthropic">${renderChoiceTextContent(content.text)}</div>`;
    } else {
      return b`<div class="json-content" data-format="object" data-content-type="anthropic">${JSON.stringify(content, null, 2)}</div>`;
    }
  }
  ...
  const renderParameterItem = (name, param, required = []) => {
    ...
    <div class="parameter-item">
      <div>
        ...
        ${param.type ? b`<span class="parameter-type">${param.type}</span>` : ""}
        ${isRequired ? b`<span class="parameter-required">required</span>` : ""}
      </div>
      ${param.description ? b`<div class="parameter-description">${param.description}</div>` : ""}
    </div>
  `;
  };
  const renderToolContent = (tool, index) => {
    if (!tool.function) {
      return b`<div class="json-content">${JSON.stringify(tool, null, 2)}</div>`;
    }
    return b`
    ${tool.function.description ? b`<div class="tool-description prose">${renderChoiceTextContent(tool.function.description)}</div>` : ""}
    ...
    <details open class="section">
      <summary class="section-header">
        <span class="section-title">
          Messages
          ${messages.length ? b`<span>(${messages.length})</span>` : ""}
        </span>
      </summary>
      <div class="section-content">
        ${!messages.length ? b`<div class="empty-state">no messages</div>` : b`${messages.map((message, index) => renderMessage(message, index))}`}
      </div>
    </details>
  `;
  };
  ...
  const openai_req_template = (obj) => b`<!DOCTYPE html>
<html lang="en-US">
  <head>
    <meta charset="UTF-8" />
    ...
    <details open class="section">
      ...
      <div class="section-content">
        <div class="usage-grid">
          ...
          ${renderUsageItem$1("Cached Tokens", usage.prompt_tokens_details?.cached_tokens)}
        </div>
      </div>
    </details>
  `;
  ...
  const renderToolCall$1 = (toolCall, index) => {
    const parsedArguments = toolCall.function?.arguments ? JSON.parse(toolCall.function.arguments) : {};
    return b`
  ...
  const renderChoiceMessageContent = (content) => {
    ...
    return b`
    <div class="prose">
      ${typeof content === "string" ? renderChoiceTextContent(content) : b`<div class="json-content">${JSON.stringify(content, null, 2)}</div>`}
    </div>
  `;
  };
  ...
  const renderChoices$1 = (choices = []) => {
    ...
    <details open class="section">
      <summary class="section-header">
        <span class="section-title">
          Choices
          ${choices.length ? b`<span>(${choices.length})</span>` : ""}
        </span>
      </summary>
      <div class="section-content">
        ${!choices.length ? b`<div class="empty-state">No choices available</div>` : b`${choices.map((choice, index) => renderChoice$1(choice, index))}`}
      </div>
    </details>
  `;
  };
  const openai_res_template = (obj) => b`<!DOCTYPE html>
<html lang="en-US">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  ...
  const renderBasicInfo = (events) => b`
  <details open class="section">
    <summary class="section-header">
...
`;
  ...
  const renderToolCall = (toolCall, index) => b`
...
`;
  const renderChoiceContent = (choice) => {
    const contentHtml = choice.content ? b`
    <div class="content-section">
      <h4>Content:</h4>
      <div class="prose">
        ${typeof choice.content === "string" ? renderChoiceTextContent(choice.content) : b`<div class="json-content">${formatJSON(choice.content)}</div>`}
      </div>
    </div>
  ` : "";
    const toolCallsHtml = choice.tool_calls?.length ? b`
    <div class="tool-calls-container">
      ${choice.tool_calls.map((toolCall, index) => renderToolCall(toolCall))}
    </div>
  ` : "";
    return b`${contentHtml}${toolCallsHtml}`;
  };
  const renderChoice = (choice, index) => b`
  <details open class="choice-item">
    <summary class="choice-header">
      <div class="choice-meta-item">
        <span class="choice-badge">Choice ${index + 1}</span>
...
`;
  ...
  <details open class="section">
    ...
    <div class="section-content">
      ${!choices?.length ? b`<div class="empty-state">No choices available</div>` : choices.map((choice, index) => renderChoice(choice, index))}
    </div>
  </details>
`;
  const renderEvent = (event, index) => b`
...
`;
  ...
  const openai_res_sse_template = (events) => b`<!DOCTYPE html>
<html lang="en-US">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
